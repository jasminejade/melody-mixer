...
def run_npy_2_mid(inputName):
    inputName=inputName
    for i in range(1):
        print(i)

        gen = np.load('exps/nowbar_hybrid/gen/gen.npy')
        tt = np.load('./preFiles_'+inputName+'/' + inputName +'x_bar_chroma_test.npy')
        madmom_cho = np.load('./preFiles_'+inputName+'/' +inputName+'y_bar_chroma_test.npy')
    ...
    for root, dirs, files in os.walk('./exps/nowbar_hybrid/gen_4dbar/all/', topdown=False):
        for name in files:
            ...
            pm.write('./output_'+inputName+'/'+name)
            i += 1

    print("-----------midi to performance midi completed !!-------------")
    print("Please find the output midi file in './output'")